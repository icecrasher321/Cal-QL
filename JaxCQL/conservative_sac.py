    ...
    def _train_step(self, train_states, target_qf_params, rng, batch, use_cql=True, cql_min_q_weight=5.0, enable_calql=False, pessimism_strategy=ImplementedPessimismStrategies.ROBUST_COVARIANCE, normalized_isolation_forest_scores=None, recon_error=None, dataset_mean=None, dataset_covinv=None):
        ...
        def loss_fn(train_params):
            ...
            if use_cql:
                ...
                elif strategy == ImplementedPessimismStrategies.AUTOENCODER_RECONSTRUCTION_ERROR:
        ...
        metrics.update(use_cql=int(use_cql), enable_calql=int(enable_calql))

        if use_cql:
            metrics.update(collect_jax_metrics(
                aux_values,
                ['cql_std_q1', 'cql_std_q2', 'cql_q1_rand', 'cql_q2_rand',
                 'cql_qf1_diff', 'cql_qf2_diff', 'cql_min_qf1_loss', 'cql_min_q_weight',
